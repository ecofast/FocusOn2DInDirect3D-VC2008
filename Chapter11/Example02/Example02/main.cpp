/*******************************************************************
程序描述: Direct3D 中的 2D 编程, 范例 11.2 之 VC++ 2008 版
程序作者: Ernest Pazera
程序功能: 矩阵变换演示
附加依赖: d3d9.lib
创建日期: 2012/01/21
...
// window title
const wchar_t* WINDOW_TITLE = L"Example 11.2 (TGO-11-C): Multiplying Transformations";
...
void Setup()
{
	...
	D3DXMatrixScaling(&matProj, 0.1875, -0.25, 1.0);

	// set the projection matrix
	g_pd3ddev->SetTransform(D3DTS_PROJECTION, &matProj);

	// set up view matrix
	D3DXMatrixTranslation(&matView, -4.0, -4.0, 0.0);

	// set the view matrix
	g_pd3ddev->SetTransform(D3DTS_VIEW, &matView);

	// set up the rotation matrix
	D3DXMatrixRotationZ(&matRotate, PI / 180.0);
