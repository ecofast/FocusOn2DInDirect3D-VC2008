/*********************************************************************************
程序描述: Direct3D 中的 2D 编程, 范例 4.2 之 VC++ 2008 版
程序作者: Ernest Pazera
程序功能: 简单演示线图元的使用
附加依赖: d3d9.lib
创建日期: 2012/12/27
...
// window title
const wchar_t* WINDOW_TITLE = L"Example 4.2 (TGO-04-D): Drawing Lines without a Vertex Buffer";
...
void Setup()
{
	...
	g_vertices[5].diffuse = D3DCOLOR_XRGB(255, 0, 0);
...
void RedrawFrame()
{
	...
	// vertex 3
	g_vertices[3].x = SCREEN_WIDTH / 2 + cos(g_angle - 2.0 * PI / 3.0) * RADIUS;
	g_vertices[3].y = SCREEN_HEIGHT / 2 + sin(g_angle - 2.0 * PI / 3.0) * RADIUS;
	// vertex 4
	g_vertices[4].x = SCREEN_WIDTH / 2 + cos(g_angle - 2.0 * PI / 3.0) * RADIUS;
	g_vertices[4].y = SCREEN_HEIGHT / 2 + sin(g_angle - 2.0 * PI / 3.0) * RADIUS;
	// vertex 5
	g_vertices[5].x = SCREEN_WIDTH / 2 + cos(g_angle) * RADIUS;
	g_vertices[5].y = SCREEN_HEIGHT / 2 + sin(g_angle) * RADIUS;
	...
	g_pd3ddev->DrawPrimitiveUP(D3DPT_LINELIST, PRIMITIVE_COUNT, g_vertices, sizeof(CustomVertex));
